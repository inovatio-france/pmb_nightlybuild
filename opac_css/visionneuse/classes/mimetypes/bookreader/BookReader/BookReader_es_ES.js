...
// $Id: BookReader_es_ES.js,v 1.2 2022/05/09 14:42:24 dgoron Exp $ */
...
BookReader.prototype.BRSearchCallback = function(results) {
    ...
    if (0 == results.matches.length) {
        var errStr  = 'No se encontraron coincidencias.';
        var timeout = 1000;
...
BookReader.prototype.initToolbar = function(mode, ui) {
    ...
    $("#BookReader").append(
          "<div id='BRtoolbar'>"
        +   "<span id='BRtoolbarbuttons'>"
        +     "<form action='javascript:br.search($(\"#textSrch\").val());' id='booksearch'><input type='search' id='textSrch' name='textSrch' val='' placeholder='Buscar dentro'/><button type='submit' id='btnSrch' name='btnSrch'>Buscar</button></form>"
...
BookReader.prototype.blankInfoDiv = function() {
    return $([
        '<div class="BRfloat" id="BRinfo">',
            '<div class="BRfloatHead">Acerca de este documento',
                '<a class="floatShut" href="javascript:;" onclick="$.fn.colorbox.close();"><span class="shift">Cerrar</span></a>',
            '</div>',
            ...
            '<div class="BRfloatFoot">',
                '<a href="http://openlibrary.org/dev/docs/bookreader" target="_blank">About the BookReader</a>',
            '</div>',
        '</div>'].join('\n')
    );
}
...
BookReader.prototype.bindNavigationHandlers = function() {
    ...
    //Reecriture PMB Services pour la visionneuse PMB
...
BookReader.prototype.buildShareDiv = function(jShareDiv)
{
    ...
    var jForm = $([
        '<p>Copie y pegue una de estas opciones para compartir este libro en otro lugar.</p>',
        '<form method="post" action="">',
            '<fieldset>',
                '<label for="pageview">Enlace a esta vista de página:</label>',
                '<input type="text" name="pageview" id="pageview" value="' + pageView + '"/>',
            '</fieldset>',
            '<fieldset>',
                '<label for="booklink">Enlace al libro:</label>',
...
BookReader.prototype.initUIStrings = function()
{
    ...
    var titles = { '.logo': '', // $$$ update after getting OL record
                   ...
                   '.onepg': 'Vista de una página',
                   '.twopg': 'Vista de dos páginas',
                   '.thumb': 'Vista en miniatura',
                   '.print': 'Imprime esta página',
                   '.embed': 'Embed BookReader',
                   '.link': 'Enlace a este libro (y página)',
                   '.bookmark': 'Marca esta pagina',
                   '.read': 'Lea este documento en voz alta',
                   '.share': 'Comparte este documento',
                   '.info': 'Acerca de este documento',
                   '.full': 'Show fullscreen',
                   '.book_left': 'Voltear a la izquierda',
                   '.book_right': 'Voltear a la derecha',
                   '.book_up': 'Página arriba',
                   '.book_down': 'Página abajo',
                   '.play': 'Jugar',
                   '.pause': 'Pausa',
                   '.BRdn': 'Mostrar/ocultar barra de navegación', // Would have to keep updating on state change to have just "Hide nav bar"
                   '.BRup': 'Mostrar/ocultar barra de navegación',
                   '.book_top': 'Primera página',
                   '.book_bottom': 'Última página'
                  };
    if ('rl' == this.pageProgression) {
        titles['.book_leftmost'] = 'Última página';
        titles['.book_rightmost'] = 'Primera página';
    } else { // LTR
        titles['.book_leftmost'] = 'Primera página';
        titles['.book_rightmost'] = 'Última página';
    }

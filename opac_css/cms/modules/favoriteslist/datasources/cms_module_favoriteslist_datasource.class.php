...
// +-------------------------------------------------+
// $Id: cms_module_favoriteslist_datasource.class.php,v 1.10 2023/10/02 14:03:37 rtigero Exp $
...
class cms_module_favoriteslist_datasource extends cms_module_common_datasource{
	...
	public function get_datas() {
	    $session_data = [];
	    $parent_data = [];
	    $parent_data = $this->get_favoriteslist_data();
		//On filtre, pour l'instant on n'a que des rubriques en favlist
		//Alors on force le filtre en sections
		//TODO A modifier quand le module evoluera
		$parent_data = $this->filter_datas("sections",$parent_data);
	    if (!empty($this->parameters["item"]) && empty($_SESSION['favorites_lists'][$this->parameters["item"]])) {
	        $_SESSION['favorites_lists'] = [
	            $this->parameters["item"] => array_values(array_intersect($this->get_default_entities_from_item($this->parameters["item"]), $parent_data))
            ];
	    }
	    $session_data = $_SESSION['favorites_lists'][$this->parameters["item"]] ?? [];
	    if (!empty($this->parameters["display_mode"]) && $this->parameters["display_mode"] == "readonly") {
	        if (!empty($_SESSION['favorites_lists'][$this->parameters["item"]])) {
	            $parent_data = $_SESSION['favorites_lists'][$this->parameters["item"]];
	        } else {
    	        $session_data = $parent_data;
	        }
	    } else {
	        foreach( $session_data as $value) {
	            $ind = array_search($value, $parent_data);
	            if ($ind !== false) {
	                unset($parent_data[$ind]);
	            }
	        }
	        $parent_data = array_merge($session_data, $parent_data);
	    }
	    $data = [
	        "display_mode" => $this->parameters["display_mode"] ?? "readonly", 
	        "ids" => $parent_data,
	        "item" => $this->parameters["item"],
	        "entities" => $session_data
	    ];
	    return $data;
	}
	
	protected function get_default_entities_from_item($item) {
	    $default_entities = [];
	    $items = $this->get_items();
	    
	    $my_item = $items[$item];
	    
	    if (!empty($my_item["cadre_content_id"])) {
	        $cms_datasource = new cms_module_favoriteslist_datasource_source_choice($my_item["cadre_content_id"]);
	        $default_entities = $cms_datasource->get_default_entities_list();
	    }
	    $default_entities = array_map("intval", $default_entities);
	    return $default_entities;
	}
	
	protected function get_favoriteslist_data() {
	    if (!empty($this->parameters['item'])) {
    	    $items = $this->get_items();
    	    $my_item = $items[$this->parameters['item']];
    	    if (!empty($my_item["cadre_content_id"]) && $my_item["cadre_parent"] != $this->cadre_parent) {
    	        $module = new cms_module_favoriteslist($my_item["cadre_parent"]);
    	        $datasource = $module->get_selected_datasource();
    	        return $datasource->get_datas()["ids"] ?? [];
    	    }
	    }
	    $parent = parent::get_datas();

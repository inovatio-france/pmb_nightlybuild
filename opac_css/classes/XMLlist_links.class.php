...
// +-------------------------------------------------+
// $Id: XMLlist_links.class.php,v 1.12 2020/04/24 07:32:34 dgoron Exp $
 ...
 // Modif Armelle Nedelec recherche de l'encodage du fichier xml et transformation en charset'
 	public function analyser() {
		...
		if ($cache_php) {
			...
			$tmp_key = $cache_php->getFromCache($key_file);
			if($tmp_key){
			    $tables = $cache_php->getFromCache($tmp_key);
			    if($tables){
					if(count($tables) == 3){
						fclose($fp);
						$this->table = $tables[0];

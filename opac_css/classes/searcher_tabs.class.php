...
// +-------------------------------------------------+
// $Id: searcher_tabs.class.php,v 1.23 2023/08/17 09:47:53 dbellamy Exp $
...
class searcher_tabs {
    ...
    protected function pager($form_name = "store_search") {
    	...
    	$nav_bar = '';
    	if($precedente >= 0)
    		$nav_bar .= "<a href='#' onClick=\"document.$form_name.page.value=$precedente; document.$form_name.submit(); return false;\"><img src='".get_url_icon('left.gif')."' style='border:0px; margin:3px 3px'  title='$msg[48]' alt='[$msg[48]]' class='align_middle'></a>";
    
    	$deb = $page_en_cours - 10 ;
    	if ($deb<0) $deb=0;
    	for($i = $deb; ($i < $this->nbepage) && ($i<$page_en_cours+10); $i++) {
    		if($i==$page_en_cours) $nav_bar .= "<strong>".($i+1)."</strong>";
    		else $nav_bar .= "<a href='#' onClick=\"document.$form_name.page.value=$i; document.$form_name.submit(); return false;\">".($i+1)."</a>";
    		if($i<$this->nbepage) $nav_bar .= " ";
    	}
    
    	if($suivante<$this->nbepage)
    		$nav_bar .= "<a href='#' onClick=\"document.$form_name.page.value=$suivante; document.$form_name.submit(); return false;\"><img src='".get_url_icon('right.gif')."' style='border:0px; margin:3px 3px' title='$msg[49]' alt='[$msg[49]]' class='align_middle'></a>";

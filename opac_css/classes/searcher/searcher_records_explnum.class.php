...
// +-------------------------------------------------+
// $Id: searcher_records_explnum.class.php,v 1.6 2024/10/16 07:48:39 dgoron Exp $
...
class searcher_records_explnum extends searcher_records
{
    ...
    {
        $final_query = 'SELECT DISTINCT id_notice AS ' . $this->object_key . ', pert FROM (' . $this->_get_search_query() . ') as lorem';
        if ($query) {
            return $final_query;
        }
    ...
    {
        return 'select DISTINCT notice_id as id_notice from notices join explnum on explnum_notice = notice_id';
    }

    public function get_full_query()
    {
        if ($this->user_query === "*") {
            return 'select DISTINCT notice_id as id_notice, 100 as pert from notices join explnum on explnum_notice = notice_id';
        }

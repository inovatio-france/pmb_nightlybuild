...
// +-------------------------------------------------+
// $Id: cms_editorial_data.class.php,v 1.21 2024/02/28 11:14:09 tsamson Exp $
        ...
        if ($this->type == "section") {
            if($this->id){
            	if ( ($gestion_acces_active==1) && ($gestion_acces_empr_cms_section==1) ) {
	                ...
	                $query.= "OR (section_start_date = 0 AND to_days(section_end_date)>=to_days(now())) ";
	                $query.= "OR (section_start_date = 0 AND section_end_date = 0)) ";
	                $query.= "AND (editorial_publication_state_opac_show=1".(empty($_SESSION['id_empr_session']) ? " AND editorial_publication_state_auth_opac_show = 0" : "").") ";
	                $query.= "ORDER BY section_order";
            	}

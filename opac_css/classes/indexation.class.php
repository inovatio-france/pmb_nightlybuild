...
// +-------------------------------------------------+
// $Id: indexation.class.php,v 1.47 2024/10/04 06:40:15 dgoron Exp $
...
class indexation {
	...
	protected function init_external_field_table($table, $k, $v) {
		...
		if($use_word){
			$full_query = $query.$jointure.$where;
		...
		if(isset($this->isbd_ask_list[$k])){ // isbd  => memo de la requete pour retrouver les id des autorités
			...
			$req=$this->get_query_select_isbd_external($id_aut).$jointure.$where;

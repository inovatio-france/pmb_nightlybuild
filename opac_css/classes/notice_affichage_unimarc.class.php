...
// +-------------------------------------------------+
// $Id: notice_affichage_unimarc.class.php,v 1.110 2023/12/22 13:36:31 dgoron Exp $
...
// 				$this->unimarc[$l->ufield][$l->field_order][$l->usubfield][$l->subfield_order];
	...
	public function affichage_avis($notice_id) {
		...
		//Affichage des Etoiles et nombre d'avis
		if ($this->avis_qte > 0) {
			$nombre_avis = "<a href='#' class='donner_avis' title=\"".htmlentities($msg['notice_title_avis'],ENT_QUOTES,$charset)."\" onclick=\"w=window.open('avis.php?todo=liste&noticeid=$notice_id','avis'); w.focus(); return false;\">".$this->avis_qte."&nbsp;".htmlentities($msg['notice_bt_avis'],ENT_QUOTES,$charset)."</a>";
			$etoiles_moyenne = $this->stars($this->avis_moyenne);
		} else {
			$nombre_avis = "<a href='#' class='donner_avis' title=\"".htmlentities($msg['notice_title_avis'],ENT_QUOTES,$charset)."\" onclick=\"w=window.open('avis.php?todo=liste&noticeid=$notice_id','avis'); w.focus(); return false;\">".htmlentities($msg['avis_aucun'],ENT_QUOTES,$charset)."</a>";
			$cpt_star = -1;
		}

		// Affichage du nombre d'avis ainsi que la note moyenne et les etoiles associees
		$img_tag .= $nombre_avis."<a href='#' class='consult_avis' title=\"".htmlentities($msg['notice_title_avis'],ENT_QUOTES,$charset)."\" onclick=\"w=window.open('avis.php?todo=liste&noticeid=$notice_id','avis'); w.focus(); return false;\">".$etoiles_moyenne."</a>";
		return $img_tag;
	}

...
// +-------------------------------------------------+
// $Id: level1_authorities_extended_search.class.php,v 1.5 2023/08/16 15:22:56 dbellamy Exp $
...
class level1_authorities_extended_search extends level1_authorities_search {
	...
	protected function get_display_link_affiliate() {
		...
		if($this->get_nb_results()){
			return "<a href='#' onclick=\"document.".$this->get_hidden_search_form_name().".action = '".$this->get_form_action()."&mode=extended_authorities&tab=catalog'; enable_operators(); document.".$this->get_hidden_search_form_name().".submit();return false;\">".$msg['suite']."&nbsp;<img src='".get_url_icon('search.gif')."' style='border:0px' /></a>";
		}else {
			return "";
		}
	}
	...
	protected function get_display_result() {
		...
		$display .= "<a href=\"#\" onclick=\"document.".$this->get_hidden_search_form_name().".action='".$this->get_form_action()."&mode=extended_authorities&count=".$this->get_nb_results()."'; enable_operators(); document.".$this->get_hidden_search_form_name().".submit(); return false;\">".$msg['suite']."&nbsp;<img src='".get_url_icon('search.gif')."' style='border:0px' /></a><br /><br />";
		return $display;
	}

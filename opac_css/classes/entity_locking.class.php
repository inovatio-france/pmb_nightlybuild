...
// +-------------------------------------------------+
// $Id: entity_locking.class.php,v 1.3 2021/02/02 11:41:38 gneveu Exp $
...
class entity_locking {
	...
	public function is_locked(){
	    ...
	    if($pmb_entity_locked_time){
	        ...
	        if(pmb_mysql_num_rows($result)){ //L'entité est verrouillée
	            ...
	            if (!$pmb_entity_locked_time || (($timestamp_locked_time + $date) < $current_time) || ($this->locked_user_id == $_SESSION['id_empr_session'])) {
	...
	public function lock_entity(){
	    global $dbh, $pmb_entity_locked_time;
	    if($pmb_entity_locked_time && $this->id){
	        //$this->get_linked_entities();
	        
	        //On commence par verrouiller l'entité de base.
	        $values = '("'.$this->id.'", "'.$this->type.'", "'.(date ("Y-m-d H:i:s")).'", "0", "0", "'.$_SESSION['id_empr_session'].'")';
	        ...
	        foreach($this->linked_entities as $entity_type => $ids){
	            foreach($ids as $id){
	                $values.= ', ("'.$id.'", "'.$entity_type.'", "'.(date ("Y-m-d H:i:s")).'", "'.$this->id.'", "'.$this->type.'", "'.$_SESSION['id_empr_session'].'")';
	            }
	        }
	        $this->locked_user_id = $_SESSION['id_empr_session'];
	        $query = 'insert into locked_entities (id_entity, type, date, parent_id, parent_type, empr_id)
				values '.$values;
	...
	public function unlock_entity($force = false){
	    if (isset($this->id) && (($this->locked_user_id == $_SESSION['id_empr_session']) || $force)) {
...
// 	    $form = str_replace('!!entity_locking_time!!', strtotime($this->locked_date)+($pmb_entity_locked_time*60), $form);
	...
	public function refresh_date(){
        ...
        where id_entity='.$this->id.' and type='.$this->type.' and empr_id = '.$this->locked_user_id.'
        or (parent_id='.$this->id.' and parent_type='.$this->type.' and empr_id = '.$this->locked_user_id.')';
	    pmb_mysql_query($query);
	}

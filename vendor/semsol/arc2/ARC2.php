...
class ARC2 {
  ...
  static function toUTF8($v) {
    ...
    $v = (strpos(encoding_normalize::utf8_decode(str_replace('?', '', $v)), '?') === false) ? encoding_normalize::utf8_decode($v) : $v;
  ...
  static function getUTF8Char($v) {
    $val = $v[1];
    if (strlen(trim($val)) === 1) return encoding_normalize::utf8_normalize($val);

...
// +-------------------------------------------------+
// $Id: selector_cms_article.class.php,v 1.1 2023/04/20 08:42:15 gneveu Exp $
if (stristr($_SERVER['REQUEST_URI'], ".class.php"))
    die("no access");

global $base_path, $class_path;

class selector_cms_article extends selector
{

    public function __construct($user_input = '')
    {
        parent::__construct($user_input);
        $this->objects_type = TYPE_CMS_ARTICLE;
    }
    ...
    protected function get_searcher_tabs_instance()
    {
        if (! isset($this->searcher_tabs_instance)) {
            $this->searcher_tabs_instance = new searcher_selectors_tabs('cms_article');
        }
    ...
    {
        $search = new search(false,"search_fields_cms_article");
    ...
    public function get_segment_type()
    {
        ...
        $this->segment_type = TYPE_CMS_ARTICLE;
        if ($this->num_segment) {
            $query = "

...
// +-------------------------------------------------+
// $Id: selector_cms_section.class.php,v 1.1 2023/04/20 08:42:15 gneveu Exp $
if (stristr($_SERVER['REQUEST_URI'], ".class.php"))
    die("no access");

global $base_path, $class_path;

class selector_cms_sections extends selector
{

    public function __construct($user_input = '')
    {
        parent::__construct($user_input);
        $this->objects_type = TYPE_CMS_SECTION;
    }
    ...
    protected function get_searcher_tabs_instance()
    {
        if (! isset($this->searcher_tabs_instance)) {
            $this->searcher_tabs_instance = new searcher_selectors_tabs('cms_section');
        }
    ...
    {
        $search = new search(false,"search_fields_cms_section");
    ...
    public function get_segment_type()
    {
        ...
        $this->segment_type = TYPE_CMS_SECTION;
        if ($this->num_segment) {
            $query = "

...
// +-------------------------------------------------+
// $Id: WYSIWYGView.php,v 1.22 2024/09/16 13:25:19 jparis Exp $
...
class WYSIWYGView extends RootView
{
	...
	public function updateViewChild($oldIdChild = 0, $idChild = 0)
	{
		...
		if ($block) {
			...
			if(!is_null($this->associatedItem)) {
	...
	public function getViewBlockById($viewId, $blocks)
	{
		...
		foreach ($blocks as $block) {
			if (in_array($block->type, $toDuplicateTypes)) {
				if(($block->viewSelected == $viewId) || (isset($block->content->viewId) && $block->content->viewId == $viewId)) {
					return $block;
				}
			}

...
// +-------------------------------------------------+
// $Id: list_indexation_ui.class.php,v 1.5 2024/10/04 12:29:37 dgoron Exp $
...
class list_indexation_ui extends list_ui {
	...
	protected function get_cell_content($object, $property) {
	    ...
	    switch($property) {
	        case 'state':
	            ...
	            $entity_number = $this->get_entity_number(static::$indexation_name);
	            if ($state) {
	        ...
	        case 'actions':
	            ...
	            //Indexer
	            $content .= "<input type='button' class='bouton_small' value='".htmlentities($msg["index"], ENT_QUOTES, $charset)."' onClick=\"document.location='".$location."&action=index'\" >";
	            break;
	        default :
	...
	public function get_entity_number($name) {
	    if(!isset($this->entities[$name]['number'])) {
	        $query = entities::get_query_count($name);
	        $this->entities[$name]['number'] = pmb_mysql_result(pmb_mysql_query($query), 0);
	    }
	    return $this->entities[$name]['number'];
	}

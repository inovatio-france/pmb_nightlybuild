...
*******************************************************************************/
// $Id: fpdf.class.php,v 1.40 2024/10/14 15:28:51 dbellamy Exp $

if (stristr($_SERVER['REQUEST_URI'], ".class.php")) die("no access");

if(!class_exists('FPDF'))
{
define('FPDF_VERSION','1.84'); //DG - MAJ 1.53 -> 1.84 avec conservation des changements
...
public $angle;
protected $missingWidth = 600;  // Largeur de caractere si non definie
...
{
	// Title of document
	$this->metadata['Title'] = $isUTF8 ? $title : encoding_normalize::utf8_normalize($title);
}

public function SetAuthor($author, $isUTF8=false)
{
	// Author of document
	$this->metadata['Author'] = $isUTF8 ? $author : encoding_normalize::utf8_normalize($author);
}

public function SetSubject($subject, $isUTF8=false)
{
	// Subject of document
	$this->metadata['Subject'] = $isUTF8 ? $subject : encoding_normalize::utf8_normalize($subject);
}

public function SetKeywords($keywords, $isUTF8=false)
{
	// Keywords of document
	$this->metadata['Keywords'] = $isUTF8 ? $keywords : encoding_normalize::utf8_normalize($keywords);
}

public function SetCreator($creator, $isUTF8=false)
{
	// Creator of document
	$this->metadata['Creator'] = $isUTF8 ? $creator : encoding_normalize::utf8_normalize($creator);
}
...
public function SetFont($family, $style='', $size=0)
{
	...
	if (!isset($this->fonts[$fontkey])) {
	    ...
	    if(array_key_exists($fontkey, $this->CoreFonts)) {
			if(!isset($fpdf_charwidths[$fontkey])) {
				//Load metric file
				if($family == 'times' || $family == 'helvetica') {
				    include($this->_getfontpath().strtolower($fontkey).'.php');
					if(!isset($fpdf_charwidths[$fontkey])) {
			...
			$this->fonts[$fontkey]=array('i'=>$i,'type'=>'Core','name'=>$this->CoreFonts[$fontkey],'up'=>-100,'ut'=>50,'cw'=>(isset($fpdf_charwidths[$fontkey]) ? $fpdf_charwidths[$fontkey] : ''));
		} else {
		    $this->Error('Undefined font: ' . $fontkey);
		}
	}
	//Select it
...
protected function _loadfont($font)
{
	...
	include($this->_getfontpath().$font);
	if(!isset($name))
...
protected function _putfonts()
{
	foreach($this->FontFiles as $file=>$info)
	{
		...
		$font = file_get_contents($this->_getfontpath().$file,true);
		if(!$font)
...
{
	$n = (!empty($this->PageInfo[1]['n']) ? $this->PageInfo[1]['n'] : 0);

...
// +-------------------------------------------------+
// $Id: mail_configuration.class.php,v 1.35 2024/09/27 12:34:34 jparis Exp $
...
class mail_configuration {
	...
	protected function build_server_content_form($interface_content_form, $readonly=false) {
	    if ($this->is_super_admin() && $this->type == "address") {
	        global $mail_configuration_allowed_unlock_form;
	        $element_lock_button = $interface_content_form->add_element('mail_configuration_lock_button', 'mail_configuration_lock_button');
	        $element_lock_button->add_html_node($mail_configuration_allowed_unlock_form);
	    }
        ...
        $element_hote->add_input_node('text', $this->hote)->set_disabled($readonly);;
        ...
        $element_port->add_input_node('integer', $this->port)->set_disabled($readonly);;
	...
	public function set_properties_from_form() {
		...
		switch ($this->type) {
			...
			case 'address':
			    ...
			    if($domain->is_allowed_hote_override() || $this->is_super_admin()) {
					$this->hote = stripslashes($mail_configuration_hote);
					$this->port = intval($mail_configuration_port);
					$this->secure_protocol = stripslashes($mail_configuration_secure_protocol);
				} else {
				...
				if($domain->is_allowed_authentification_override() || $this->is_super_admin()) {
					$this->user = stripslashes($mail_configuration_user);
					if($mail_configuration_password != '') {
						$this->password = stripslashes($mail_configuration_password);
					}

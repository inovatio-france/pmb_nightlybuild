...
// +-------------------------------------------------+
// $Id: log.class.php,v 1.17 2024/09/25 15:12:51 dgoron Exp $
...
class log {
	...
	protected static $purge_max_lines = 50000; // 50 000 lignes max dans la table
	...
	public static function purge() {
		...
		$count = pmb_mysql_result(pmb_mysql_query("SELECT count(1) FROM logs"), 0);
		if($count >= static::$purge_max_lines) {
		    //Purge des 20% de logs les plus anciens + la différence entre le nbre de logs et le nbre max
		    $purge_nb_lignes = intval(20/100*static::$purge_max_lines)+($count - static::$purge_max_lines);
		    $query = "DELETE FROM logs ORDER BY log_date LIMIT ".$purge_nb_lignes;
		    pmb_mysql_query($query, static::get_dbh());
		}
	}

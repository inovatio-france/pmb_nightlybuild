...
// +-------------------------------------------------+
// $Id: mail_opac_user_resa.class.php,v 1.4 2023/10/24 09:57:08 gneveu Exp $
...
class mail_opac_user_resa extends mail_opac_user {
	...
	protected function get_gestion_permalink() {
		...
		$permalink = '';
		if ($this->id_notice) {
		    $this->id_notice = intval($this->id_notice);
			$niveau_biblio = notice::get_niveau_biblio($this->id_notice);
			switch ($niveau_biblio) {
				case 'a':
					$query = "SELECT analysis_bulletin FROM analysis WHERE analysis_notice = ".$this->id_notice;
					$bul_id = pmb_mysql_result(pmb_mysql_query($query), 0, 0);
					$permalink = $pmb_url_base."catalog.php?categ=serials&sub=analysis&action=analysis_form&bul_id=".intval($bul_id)."&analysis_id=".$this->id_notice;
					break;
				case 's':
					$permalink = $pmb_url_base."catalog.php?categ=serials&sub=view&serial_id=".$this->id_notice;
					break;
				case 'm':
				default:
					$permalink = $pmb_url_base."catalog.php?categ=isbd&id=".$this->id_notice;
					break;
			}
		} else {
	...
	protected function get_mail_content() {
		...
		$mail_content .= "<hr />";
		//On récupère l'affichage des notices dans un contexte de gestion
		//Utilisé notamment par les webservices
		if (SESSrights & CATALOGAGE_AUTH) {
			$mail_content .= "<a href='".$this->get_gestion_permalink()."'>".reservation::get_notice_title($this->id_notice, 0)."</a>";
		} else {
			$mail_content .= reservation::get_notice_title($this->id_notice, 0);
		}

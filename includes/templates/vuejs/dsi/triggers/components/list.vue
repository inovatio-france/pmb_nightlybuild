<template>
	<div id="event-list">
				...
				<table>
					...
					<tbody>
						<tr v-for="(event, index) in list" :key="index">
							...
							<td style="cursor: pointer" @click="edit(event.id)">
								<a href="#" @click.prevent="edit(event.id)">{{ event.name }}</a>
							</td>
							<td>{{ event.type }}</td>
							<td>
								<span v-for="(tag, i) in event.tags" :key="i">
									{{ tag.name }}
									<span v-if="(i + 1) < event.tags.length">, </span>
								</span>
							</td>
							<td class="dsi-table-right">
								<button type="button" class="bouton" @click="exportTrigger(event.id)">{{ messages.get('dsi', 'model_export') }}</button>
								<button type="button" class="bouton" @click="edit(event.id)">{{ messages.get('dsi', 'edite') }}</button>
								<button type="button" class="bouton" @click="duplicate(event.id)">{{ messages.get('dsi', 'dsi_duplicate') }}</button>
								<button type="button" class="bouton btnDelete" @click="deleteEvent(event)">{{ messages.get('dsi', 'del') }}</button>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<div class="dsi-list-actions">
			...
			<modelImport @importModel="importTrigger"></modelImport>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
            ...
            getTypeFromId: function(id) {
				let find = this.types.find(type => type.id == id);
				if(find) {
					return find.name;
				}
				return "";
		    ...
		    add: function () {
		        document.location = "./dsi.php?categ=triggers&action=add";
		    },
		    edit: function (id) {
		        document.location = `./dsi.php?categ=triggers&action=edit&id=${id}`;
		    },
			duplicate : async function(idEvent) {
				let duplicate = await this.ws.post(this.$root.categ, "duplicate", {"id" : idEvent});
				if(!duplicate.error) {
					this.edit(duplicate.id);
					//this.$set(this.originalList, this.originalList.length, this.formatListElement(duplicate));
				}
			},
			exportTrigger: async function(idTrigger) {
				window.open(`./rest.php/dsi/triggers/export/${idTrigger}`, "__blank");
			},
			importTrigger: function(importedTrigger) {
				this.$set(this.originalList, this.originalList.length, this.formatListElement(importedTrigger));
			},
			deleteEvent : async function(event) {
				if (confirm(this.messages.get('dsi', 'confirm_del'))) {
					let response = await this.ws.post("triggers", 'delete', event);
					if (response.error) {
						this.notif.error(this.messages.get('dsi', response.errorMessage));
					} else {
						let i = this.originalList.findIndex((d) => d.id == event.id);
			...
			execAction: async function(action) {
				...
				switch(action) {
					case "deleteAll":
						if(confirm(this.messages.get('dsi', 'confirm_del'))) {
							response = await this.ws.post("triggers", "deleteAll", { ids: this.batchActionList });
							if (!response.error) {

<template>
	<div id="view-list">
		...
		:nbResultDisplay="false">
			<template #content="{ list }">
				<table>
					...
					<tbody>
						<tr v-for="(view, index) in list" :key="index">
							<td>
								<input type="checkbox"
							...
							<td style="cursor: pointer" @click="edit(view.id)">
								<a href="#" @click.prevent="edit(view.id)">{{ view.name }}</a>
							</td>
							<td>{{ view.type }}</td>
							<td>
								<span v-for="(tag, i) in view.tags" :key="i">
									{{ tag.name }}
									<span v-if="(i + 1) < view.tags.length">, </span>
								</span>
							</td>
							<td class="dsi-table-right">
								<button type="button" class="bouton" @click="exportView(view.id)">{{ messages.get('dsi', 'model_export') }}</button>
								<button type="button" class="bouton" @click="edit(view.id)">{{ messages.get('dsi', 'edite') }}</button>
								<button type="button" class="bouton" @click="duplicate(view.id)">{{ messages.get('dsi', 'dsi_duplicate') }}</button>
								<button type="button" class="bouton btnDelete" @click="deleteView(view)">{{ messages.get('dsi', 'del') }}</button>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<div class="dsi-list-actions">
			...
			<modelImport @importModel="importView"></modelImport>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
		    ...
		    edit: function (id) {
		        document.location = `./dsi.php?categ=views&action=edit&id=${id}`;
		    },
			duplicate : async function(idView) {
				let duplicate = await this.ws.post(this.$root.categ, "duplicate", {"id" : idView});
				if(!duplicate.error) {
					this.edit(duplicate.id);
					//this.$set(this.originalList, this.originalList.length, this.formatListElement(duplicate));
				}
			},
			exportView: async function(idView) {
				window.open(`./rest.php/dsi/views/export/${idView}`, "__blank");
			},
			importView: function(importedView) {
				this.$set(this.originalList, this.originalList.length, this.formatListElement(importedView));
			},
			deleteView : async function(view) {
				if (confirm(this.messages.get('dsi', 'confirm_del'))) {
					let response = await this.ws.post("views", 'delete', view);
					if (response.error) {
						this.notif.error(this.messages.get('dsi', response.errorMessage));
					} else {
						let i = this.originalList.findIndex((d) => d.id == view.id);
			...
			execAction: async function(action) {
				...
				switch(action) {
					case "deleteAll":
						if(confirm(this.messages.get('dsi', 'confirm_del'))) {
							response = await this.ws.post("views", "deleteAll", { ids: this.batchActionList });
							if (!response.error) {

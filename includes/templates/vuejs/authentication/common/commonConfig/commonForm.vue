<template>
    <div>
        <form class="form-admin" action="" method="POST" @submit.prevent="save">
               <div class="form-contenu">
                 ...
                 <div class="row">
                        ...
                        <div class="colonne_suite">
                            <input type="checkbox" id="context_opac" name="context_opac" value="1" v-model="modelParams.context_opac" disabled/>
                            <label class="etiquette" for="context_opac" >{{ data.messages.context_opac}}</label>
                            <input type="checkbox" id="context_gestion" name="context_gestion" value="1" v-model="modelParams.context_gestion" disabled/>
                ...
                <div class="row">
                    <div class="colonne3">
                       <label class="etiquette" for="template">{{ data.messages["template"] }}</label>
                    </div>
                    <div class="colonne_suite">
                ...
                <template v-if="1 == modelParams.context_gestion">
                    ...
                    <div class="row">
                        ...
                        <div class="colonne_suite">
                            ...
                            <button class="bouton" type="button" @click.prevent="updateModelSelectedAttrsUser(selectedAttrUser)">
                                <i class="fa fa-plus"></i>
                            </button>
                        </div>
                    </div>
                ...
                <!-- Boutons -->
                <div class="row">
                    <button type="button" class="bouton btnCancel" @click.prevent="cancel">{{ messages.get("common", "cancel") }}</button>
                    <button class="bouton" type="submit">{{ messages.get("common", "submit") }}</button>
                    <button type="button" class="bouton right" v-if="0 !== modelParams.id" @click.prevent="deleteConfig">{{ messages.get("common", "remove") }}</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    import openIDConnectForm from "../commonService/openIDConnectForm.vue";
    ...
    export default {
      ...
      created: function() {
          ...
          if(this.data.model) {
                ...
                this.$set(this, "modelUserCreateClass", settings.emprCreateClass ?? "");
        ...
        methods: {
          ...
          save: async function () {
               ...
               let response = await this.ws.post(sourceName, "saveConfig", {
                   modelParams: param,
           ...
           deleteConfig: async function () {
               let response = await this.ws.get("deleteConfigForm", this.modelParams.id);
               if (response.error) {
                   if (response.errorMessage) {
                       console.error(response.errorMessage);
                   }
                   ...
                   this.notif.info(this.messages.get("common", "remove"));
                   this.$root.$emit('deleteConfigEvent');
                   this.$root.$emit('updateConfigsList');
               }
           },
            ...
            addEmprCreateClass: function (className) {
                if("" == className) {
                    return;
                }
                let found = this.modelEmprCreateClass.find(element => element == className);
                if (undefined != found) {
                    return;
                }
                this.modelEmprCreateClass.push(className);
                this.selectedEmprCreateClass = '';
            },

            removeEmprCreateClass: function (className) {
                if("" == className) {
                    return;
                }
                this.modelEmprCreateClass = this.modelEmprCreateClass.filter(element => element != className);
                return;
            },
            ...
            addUserCreateClass: function (className) {
                if("" == className) {
                    return;
                }
                let found = this.modelUserCreateClass.find(element => element == className);
                if (undefined != found) {
                    return;
                }
                this.modelUserCreateClass.push(className);
                this.selectedUserCreateClass = '';
            },

            removeUserCreateClass: function (className) {
                if("" == className) {
                    return;
                }
                this.modelUserCreateClass = this.modelUserCreateClass.filter(element => element != className);
                return;
            },

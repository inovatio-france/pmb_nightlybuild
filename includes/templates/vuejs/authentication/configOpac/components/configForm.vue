              ...
              <tr class="source-item-name">
	              ...
	              <td class="source-item-name">
                       <input 
                            ...
                            id="automatic-active" 
                            v_model="allowInternalOpacAuthentication"
                            @click="updateAllowInternalOpac"
                            :checked="(allowInternalOpacAuthentication ? 'checked' : '')"
                            />
...
<script>
	...
	export default {
		props : [
			...
			"configs_list",
			"models_list",
			"allow_internal_opac_authentication"
			],
		data: function () {
			return {
			    hover: null,
			    configsList: [],
			    selectedModel: 0,
	             allowInternalOpacAuthentication: 0
			}
		},
		...
		created: function() {
		    ...
		    this.allowInternalOpacAuthentication = this.allow_internal_opac_authentication;
		},
		...
		methods: {
            ...
            updateAllowInternalOpac: async function() {
            	let response = await this.ws.get("updateAllowInternalOpac", this.allowInternalOpacAuthentication);
            	this.$set(this, "allowInternalOpacAuthentication", response.state);
            }
		}
	}
</script>

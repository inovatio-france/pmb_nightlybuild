...
// +-------------------------------------------------+
// $Id: scheduler_caddie_planning.class.php,v 1.17 2024/02/14 10:22:01 dgoron Exp $
		...
		if (!empty($liste)) {
			...
			foreach($print_cart as $key => $cart_type) {
				...
				foreach($cart_type["classement_list"] as $keyBis => $cart_typeBis) {
					$display.=gen_plus($key.$keyBis,$cart_typeBis["titre"],"<table style='border:0px; border-spacing: 0px; width: 100%' class='classementGen_tableau'>".$cart_typeBis["cart_list"]."</table>",0);
				}
			}
		} else {
			$display .= $msg[398];

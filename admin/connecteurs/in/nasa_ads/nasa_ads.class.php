...
// +-------------------------------------------------+
// $Id: nasa_ads.class.php,v 1.10 2023/08/28 14:04:12 tsamson Exp $
	...
	public function search($source_id,$query,$search_id) {
		...
		configurer_proxy_curl($ch,$addr);	
 		...
 		if (!$cexec) {
 			...
 			if (strtoupper($charset)!="UTF-8") $cexec = encoding_normalize::utf8_decode($cexec);	
	 		...
	 		foreach($notices_html as $notice_html){
				...
				configurer_proxy_curl($ch_notice,$url_notice);	
		 		...
		 		if (!$cexec_notice) {
		 		    ...
		 		    if (strtoupper($charset)!="UTF-8") $cexec_notice = encoding_normalize::utf8_decode($cexec_notice);	
